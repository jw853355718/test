...
ss_tproxy_auser=`nvram get ss_tproxy_auser`
	if [ "Sh10_clash.sh" != "$ss_tproxy_auser" ] && [ "" != "$ss_tproxy_auser" ] ; then
		logger -t "$scriptname[$$]" "错误！！！由于已启用 $ss_tproxy_auser 透明代理，停止启用 clash 透明代理！"
...
if [ "$1" = "x" ] ; then
	if [ -f $relock ] ; then
		logger -t "$scriptname[$$]" "多次尝试启动失败，等待【"`cat $relock`"分钟】后自动尝试重新启动"
		exit 0
	fi
...
if [ "$A_restart" != "$B_restart" ] ; then
	...
	if [ -z "$clash_wget_yml" ] ; then
		...
		logger -t "$scriptname[$$]" "停止 clash 服务器订阅"
	else
		if [ "$app_120" == "1" ] ; then
			cru.sh a clash_link_update "24 */6 * * * $scriptfilepath wget_yml &" &
			logger -t "$scriptname[$$]" "启动 clash 服务器订阅，添加计划任务 (Crontab)，每6小时更新"
		else
			cru.sh d clash_link_update
		fi
	fi
...
if [ "$D_restart" != "$C_restart" ] ; then
	nvram set clash_2_status=$C_restart
	needed_2_restart=1
else
	needed_2_restart=0
fi
...
if [ "$clash_enable" != "1" ] && [ "$needed_restart" = "1" ] ; then
	[ ! -z "`pidof clash`" ] && logger -t "$scriptname[$$]" "停止 clash" && clash_close
...
clash_keep () {
logger -t "$scriptname[$$]" "守护进程启动"
/etc/storage/script/sh_ezscript.sh 3 & #更新按钮状态
if [ -s /tmp/script/_opt_script_check ]; then
SVC_PATH="$(which clash)"
sed -Ei '/【clash】|^$/d' /tmp/script/_opt_script_check
cat >> "/tmp/script/_opt_script_check" <<-OSC
	[ -z "\`pidof clash\`" ] || [ ! -s "$SVC_PATH" ] && nvram set clash_status=00 && logger -t "$scriptname[$$]" "重新启动" && eval "$scriptfilepath &" && sed -Ei '/【clash】|^$/d' /tmp/script/_opt_script_check # 【clash】
OSC
#return
fi
clash_enable=`nvram get app_88`
...
if [ ! -s "$SVC_PATH" ] ; then
	logger -t "$scriptname[$$]" "找不到 $SVC_PATH，安装 opt 程序"
...
if [ "$app_78" == "premium" ] || [ "$app_78" == "premium_1" ] ; then
	[ ! -s "$SVC_PATH" ] && logger -t "$scriptname[$$]" "下载 premium (闭源版) 主程序: https://github.com/Dreamacro/clash/releases/tag/premium" && [ "$app_78" != "premium_1" ] && nvram set app_78="premium_1" && app_78="premium_1"
	wgetcurl_file "$SVC_PATH" "$hiboyfile/clash-premium" "$hiboyfile2/clash-premium"
else
	[ ! -s "$SVC_PATH" ] && logger -t "$scriptname[$$]" "下载支持Vless+XTLS的CDN核心 experimental 主程序: https://github.com/ClashDotNetFramework/experimental-clash" && [ "$app_78" != "experimental_1" ] && nvram set app_78="experimental_1" && app_78="experimental_1"
...
if [ ! -s "$SVC_PATH" ] ; then
	logger -t "$scriptname[$$]" "找不到 $SVC_PATH ，需要手动安装 $SVC_PATH"
	logger -t "$scriptname[$$]" "启动失败, 10 秒后自动尝试重新启动" && sleep 10 && clash_restart x
fi
if [[ "$(yq -h 2>&1 | wc -l)" -lt 2 ]] ; then
	yq_check
if [[ "$(yq -h 2>&1 | wc -l)" -lt 2 ]] ; then
	logger -t "$scriptname[$$]" "找不到 /opt/bin/yq ，需要手动安装 /opt/bin/yq"
	logger -t "$scriptname[$$]" "启动失败, 10 秒后自动尝试重新启动" && sleep 10 && clash_restart x
fi
fi
...
logger -t "$scriptname[$$]" "调整 /tmp 挂载分区的大小， /opt 可用空间： $Available_A → $Available_B M"
fi
clash_get_clash_webs
# 下载clash_webs
if [ ! -d "/opt/app/clash/clash_webs" ] ; then
	if [ "$app_79" == "clash" ] || [ "$app_79" == "clash_1" ] ; then
		logger -t "$scriptname[$$]" "下载 yacd 面板 Source: https://github.com/haishanh/yacd"
		...
		logger -t "$scriptname[$$]" " 下载 clash 面板 : http://clash.razord.top/"
	...
	[ -d "/opt/app/clash/clash_webs" ] && logger -t "$scriptname[$$]" "下载 clash_webs 完成"
fi

if [ ! -s /opt/app/clash/config/Country.mmdb ] ; then
logger -t "$scriptname[$$]" "初次启动会自动下载 geoip 数据库文件：/opt/app/clash/config/Country.mmdb"
logger -t "$scriptname[$$]" "备注：如果缺少 geoip 数据库文件会启动失败，需 v0.17.1 或以上版本才能自动下载 geoip 数据库文件"
...
if [ "$NUM" -ge "3" ] && [ "$su_x" = "1" ] ; then
	...
	addgroup -g 1333 clashy
	adduser -G clashy -u 1334 clashy  -D -S -H -s /bin/sh
	su_cmd3="su clashy -c "
	sed -Ei s/1321:1321/0:1321/g /etc/passwd
	sed -Ei s/1334:1333/0:1333/g /etc/passwd
	su_cmd="su ‍✈️ -c "
	gid_owner="1321"
fi
...
if [ "$ss_udp_enable" = "1" ] || [ "$app_114" = "0" ] ; then
	[ "$su_x" != "1" ] && logger -t "$scriptname[$$]" "缺少 su 命令"
	[ "$NUM" -ge "3" ] || logger -t "$scriptname[$$]" "缺少 iptables -m owner 模块"
	if [ "$NUM" -ge "3" ] && [ "$su_x" = "1" ] ; then
...
[ "$ss_udp_enable" = "0" ] && logger -t "$scriptname[$$]" "仅代理 TCP 流量"
[ "$ss_udp_enable" = "1" ] && logger -t "$scriptname[$$]" "代理 TCP 和 UDP 流量"
[ "$app_114" = "0" ] && logger -t "$scriptname[$$]" "启动路由自身流量走透明代理"
[ "$app_114" = "1" ] && logger -t "$scriptname[$$]" "停止路由自身流量走透明代理"
fi
logger -t "$scriptname[$$]" "运行 $SVC_PATH"
...
sleep 7
[ ! -z "`pidof clash`" ] && logger -t "$scriptname[$$]" "启动成功" && clash_restart o
[ -z "`pidof clash`" ] && logger -t "$scriptname[$$]" "启动失败, 注意检clash是否下载完整,10 秒后自动尝试重新启动" && sleep 10 && clash_restart x
clash_get_status
if [ "$clash_mixed" == "1" ] ; then
logger -t "$scriptname[$$]" "双开 clash 开启 mixed 代理"
#su_cmd3="su clashy -c "
su_cmd4="$SVC_PATH -d /opt/app/clash/config2"
eval "$su_cmd3" '"cmd_name=clash && '"$su_cmd4"' $cmd_log"' &
...
if [ "$app_114" = 0 ] ; then
logger -t "$scriptname[$$]" "同时将透明代理规则应用到 OUTPUT 链, 让路由自身流量走透明代理"
fi
logger -t "$scriptname[$$]" "完成 透明代理 转发规则设置"
if [ "$chinadns_enable" != "0" ] || [ "$chinadns_ng_enable" != "0" ] ; then
logger -t "$scriptname[$$]" "已经启动 chinadns 防止域名污染"
else
logger -t "$scriptname[$$]" "启动 clash DNS 防止域名污染【端口 ::1#8053】"
fi
restart_dhcpd
logger -t "$scriptname[$$]" "启动后若发现一些网站打不开, 估计是 DNS 被污染了. 解决 DNS 被污染方法："
logger -t "$scriptname[$$]" "①电脑设置 DNS 自动获取路由 ip。检查 hosts 是否有错误规则。"
logger -t "$scriptname[$$]" "②电脑运行 cmd 输入【ipconfig /flushdns】, 清理浏览器缓存。"
# 透明代理
fi
...
if [ "$1" != "$ss_tproxy_auser" ] ; then
	logger -t "$scriptname[$$]" "脚本 [Sh99_ss_tproxy.sh] 当前使用者: $auser_b ，跳过 $auser_a 的运行命令"
	logger -t "$scriptname[$$]" "需要停用 $auser_b 后才能使用 $auser_a 运行 [Sh99_ss_tproxy.sh] 脚本"
	return
fi
lan_ipaddr=`nvram get lan_ipaddr`
ss_tproxy_mode_x=`nvram get app_110`
[ -z $ss_tproxy_mode_x ] && ss_tproxy_mode_x=0 && nvram set app_110=0
[ "$ss_tproxy_mode_x" = "0" ] && logger -t "$scriptname[$$]" "【自动】设置 ss_tproxy 配置文件，配置导入中..."
[ "$ss_tproxy_mode_x" = "1" ] && logger -t "$scriptname[$$]" "【手动】设置 ss_tproxy 配置文件，跳过配置导入" && return
...
logger -t "$scriptname[$$]" "【自动】设置 ss_tproxy 配置文件，完成配置导入"
}
...
[ -z "$clash_wget_yml" ] && logger -t "$scriptname[$$]" "找不到 【订阅链接】，需要手动填写" && return
if [[ "$(yq -h 2>&1 | wc -l)" -lt 2 ]] ; then
	yq_check
if [[ "$(yq -h 2>&1 | wc -l)" -lt 2 ]] ; then
	logger -t "$scriptname[$$]" "找不到 /opt/bin/yq ，需要手动安装 /opt/bin/yq"
...
mkdir -p /tmp/clash
logger -t "$scriptname[$$]" "服务器订阅：开始更新"
yml_tmp="/tmp/clash/app_20.sh"
rm -f $yml_tmp
if [ ! -z "$(echo "$clash_wget_yml" | grep '^/')" ] ; then
[ -f "$clash_wget_yml" ] && cp -f "$clash_wget_yml" "$yml_tmp"
[ ! -f "$clash_wget_yml" ] && logger -t "$scriptname[$$]" "错误！！ $clash_wget_yml 文件不存在！"
else
if [ -z  "$(echo "$clash_wget_yml" | grep 'http:\/\/')""$(echo "$clash_wget_yml" | grep 'https:\/\/')" ]  ; then
	logger -t "$scriptname[$$]" "$clash_wget_yml"
	logger -t "$scriptname[$$]" "错误！！clash 服务器订阅文件下载地址不含http(s)://！请检查下载地址"
	return
fi
...
if [ ! -s $yml_tmp ] ; then
	rm -f $yml_tmp
	logger -t "$scriptname[$$]" "错误！！clash 服务器订阅文件获取失败！请检查地址"
	return
else
	dos2unix $yml_tmp
	...
	#rm_temp
	logger -t "$scriptname[$$]" "下载 clash 配置完成！"
	rm -f $yml_tmp
fi
[ "$app_120" == "2" ] && nvram set clash_status=wget_yml
logger -t "$scriptname[$$]" "服务器订阅：更新完成"
logger -t "$scriptname[$$]" "请按F5或刷新 web 页面刷新配置"
}

yq_check () {

if [[ "$(yq -h 2>&1 | wc -l)" -lt 2 ]] ; then
	logger -t "$scriptname[$$]" "找不到 yq，安装 opt 程序"
	/etc/storage/script/Sh01_mountopt.sh start
if [[ "$(yq -h 2>&1 | wc -l)" -lt 2 ]] ; then
	for h_i in $(seq 1 2) ; do
...
if [[ "$(yq -h 2>&1 | wc -l)" -lt 2 ]] ; then
	...
	#opkg install yq
if [[ "$(yq -h 2>&1 | wc -l)" -lt 2 ]] ; then
	logger -t "$scriptname[$$]" "找不到 yq，需要手动安装 opt 后输入[opkg update; opkg install yq]安装"
	return 1
fi
fi
fi
...
if [ ! -z "$Proxy_txt" ] ; then
logger -t "$scriptname[$$]" "把 DNS 地址加入规则！ $Proxy_txt ：8.8.8.8,8.8.4.4,208.67.222.222,208.67.220.220"
...
logger -t "$scriptname[$$]" "更新下载 GeoIP2国家数据库 数据库文件"
/etc/storage/script/Sh01_mountopt.sh start
...
logger -t "$scriptname[$$]" "不改写配置，直接使用原始配置启动！（有可能端口不匹配导致功能失效）"
logger -t "$scriptname[$$]" "请手动修改配置， HTTP 代理端口：7890"
logger -t "$scriptname[$$]" "请手动修改配置， SOCKS5 代理端口：7891"
logger -t "$scriptname[$$]" "请手动修改配置，透明代理端口：7892"
cp -f /etc/storage/app_20.sh /opt/app/clash/config/config.yaml
else
 # 改写配置适配脚本
logger -t "$scriptname[$$]" "初始化 clash dns 配置"
mkdir -p /tmp/clash
...
logger -t "$scriptname[$$]" "变更 clash dns 端口 listen 0.0.0.0:8054 自动开启第三方 DNS 程序"
yq w -i $config_dns_yml dns.listen 0.0.0.0:8054
rm_temp
dns_start_dnsproxy='0' # 0:自动开启第三方 DNS 程序(dnsproxy) ;
else
logger -t "$scriptname[$$]" "变更 clash dns 端口 listen 0.0.0.0:8053 跳过自动开启第三方 DNS 程序但是继续把DNS绑定到 8053 端口的程序"
yq w -i $config_dns_yml dns.listen 0.0.0.0:8053
rm_temp
dns_start_dnsproxy='1' # 1:跳过自动开启第三方 DNS 程序但是继续把DNS绑定到 8053 端口的程序
fi
if [ ! -s $config_dns_yml ] ; then
logger -t "$scriptname[$$]" "yq 初始化 clash dns 配置错误！请检查配置！"
logger -t "$scriptname[$$]" "恢复原始 clash dns 配置！"
rm -f /etc/storage/app_21.sh
initconfig
cp -f /etc/storage/app_21.sh $config_dns_yml

fi
logger -t "$scriptname[$$]" "初始化 clash 配置"
...
logger -t "$scriptname[$$]" "配置 clash 添加本地代理节点"
[ ! -z "$(yq -V 2>&1 | grep 3\.4\.1)" ] && source /etc/storage/app_33.sh
if [ ! -s $config_yml ] ; then
logger -t "$scriptname[$$]" "yq 添加本地代理节点 配置错误！请检查配置！"
cp -f /etc/storage/app_20.sh $config_yml
fi
fi
sed -Ei '/^$/d' $config_yml
...
rm_temp
logger -t "$scriptname[$$]" "允许局域网的连接"
if [ "$clash_http_enable" != "0" ] ; then
yq w -i $config_yml port 7890
rm_temp
logger -t "$scriptname[$$]" "HTTP 代理端口：7890"
else
...
rm_temp
logger -t "$scriptname[$$]" "SOCKS5 代理端口：7891"
else
...
rm_temp
logger -t "$scriptname[$$]" "redir 代理端口：7892"
else
yq d -i $config_yml redir-port
rm_temp
fi
logger -t "$scriptname[$$]" "删除 Clash 配置文件中原有的 DNS 配置"
...
rm_temp
# 转换新参数兼容 1.0 或更高版本
[ ! -z "$(cat $config_yml | grep "Proxy:" )" ] && sed -e 's@Proxy:@proxies:@g' -i $config_yml && logger -t "$scriptname[$$]" "转换新参数: Proxy --> proxies"
[ ! -z "$(cat $config_yml | grep "Proxy Group:" )" ] && sed -e 's@Proxy Group:@proxy-groups:@g' -i $config_yml && logger -t "$scriptname[$$]" "转换新参数: Proxy Group --> proxy-groups"
[ ! -z "$(cat $config_yml | grep "Rule:" )" ] && sed -e 's@Rule:@rules:@g' -i $config_yml && logger -t "$scriptname[$$]" "转换新参数: Rule --> rules"
[ ! -z "$(cat $config_yml | grep "proxy-provider:" )" ] && sed -e 's@proxy-provider:@proxy-providers:@g' -i $config_yml && logger -t "$scriptname[$$]" "转换新参数: proxy-provider --> proxy-providers"
# 设置日志输出 silent / info / warning / error / debug(默认[error]，避免日志内容过多导致卡机）。
logger -t "$scriptname[$$]" "设置日志输出: $log_level"
yq w -i $config_yml log-level $log_level
if [ ! -s $config_yml ] ; then
logger -t "$scriptname[$$]" "yq 初始化 clash 配置错误！请检查配置！"
logger -t "$scriptname[$$]" "尝试直接使用原始配置启动！"
cp -f /etc/storage/app_20.sh $config_yml
else
logger -t "$scriptname[$$]" "将 DNS 配置 /tmp/clash/dns.yml 以覆盖的方式与 $config_yml 合并"
cat /tmp/clash/dns.yml >> $config_yml
#yq m -x -i $config_yml /tmp/clash/dns.yml
#rm_temp
#merge_dns_ip
logger -t "$scriptname[$$]" "初始化 clash 配置完成！实际运行配置：/opt/app/clash/config/config.yaml"
update_2_yml
fi
fi
}
...
logger -t "$scriptname[$$]" "初始化 clash 双开 mixed 代理"
...
sed -i 's/MATCH,A0/MATCH,B0/' $config_2_yml

logger -t "$scriptname[$$]" "初始化 clash 双开 配置完成！实际运行配置：/opt/app/clash/config2/config.yaml"
...
if [ -z "$curltest" ] || [ ! -s "`which curl`" ] ; then
	logger -t "$scriptname[$$]" "找不到 curl ，安装 opt 程序"
	/etc/storage/script/Sh01_mountopt.sh optwget
	curltest=`which curl`
	if [ -z "$curltest" ] || [ ! -s "`which curl`" ] ; then
		logger -t "$scriptname[$$]" "找不到 curl ，需要手动安装 opt 后输入[opkg update; opkg install curl]安装"
		eturn 1
	fi
fi
...
# jq_check
# if [[ "$(jq -h 2>&1 | wc -l)" -lt 2 ]] ; then
	# logger -t "$scriptname[$$]" "错误！找不到 jq 程序"
	# return 1
# fi
# fi
...
# logger -t "$scriptname[$$]" "保存web节点选择，完成！"
# fi
# if [ "$1" == "set" ] ; then
# logger -t "$scriptname[$$]" "恢复web节点选择"
...
# logger -t "$scriptname[$$]" "恢复web节点选择，完成！"
# fi
if [ "$1" == "reload" ] ; then
logger -t "$scriptname[$$]" "api热重载配置"
...
logger -t "$scriptname[$$]" "api热重载配置，完成！"
fi
...
logger -t "$scriptname[$$]" "更换 premium (闭源版) 主程序: https://github.com/Dreamacro/clash/releases/tag/premium"
fi
if [ "$app_78" == "experimental" ] ; then
link_get="clash"
nvram set app_78="experimental_1" ; app_78="experimental_1"
logger -t "$scriptname[$$]" "更换支持Vless+XTLS的CDN核心 experimental 主程序: https://github.com/ClashDotNetFramework/experimental-clash"
fi
if [ ! -z "$link_get" ] ; then
SVC_PATH="$(which clash)"
[ ! -s "$SVC_PATH" ] && SVC_PATH="/opt/bin/clash"
rm -rf "$SVC_PATH" /opt/opt_backup/bin/clash
logger -t "$scriptname[$$]" "自动下载 clash 主程序"
...
logger -t "$scriptname[$$]" "更换 yacd 面板 Source: https://github.com/haishanh/yacd"
fi
if [ "$app_79" == "clash" ] ; then
link_get="clash_webs2.tgz"
nvram set app_79="clash_1" ; app_79="clash_1"
logger -t "$scriptname[$$]" " 更换 clash 面板 : http://clash.razord.top/"
fi
if [ ! -z "$link_get" ] ; then
# 下载clash_webs
...
logger -t "$scriptname[$$]" "下载 clash_webs 完成"
logger -t "$scriptname[$$]" "需按 Ctrl + F5 强制刷新 web 面板"
...
updateapp18)
	clash_restart o
	[ "$clash_enable" = "1" ] && nvram set clash_status="updateclash" && logger -t "$scriptname[$$]" "重启" && clash_restart
	[ "$clash_enable" != "1" ] && nvram set clash_v="" && logger -t "$scriptname[$$]" "更新" && update_app del
	;;
update_app)
	update_app
	;;
