...
if [ ! -d /opt/app/ss_tproxy ] ; then
	logger -t "$scriptname[$$]" "找不到 /opt/app/ss_tproxy ，安装 opt 程序"
...
echo "create proxyaddr hash:net hashsize 256 family inet counters
create proxyaddr6 hash:net hashsize 256 family inet6
...
update_gfwlist_file() {
	logger -t "$scriptname[$$]" "开始下载更新 gfwlist 文件...."
	...
	if [ "$ss_3p_gfwlist" = "1" ] ; then
		logger -t "$scriptname[$$]" "正在获取官方 gfwlist...."
		...
		[ -z "$(cat $tmp_down_file | grep google )" ] && { rm -f $tmp_down_file ; logger -t "$scriptname[$$]" "错误！！！找不到 google ，base64 解码官方 gfwlist 数据不完整" ; }
		[ -z "$(cat $tmp_down_file | grep '\-\-\-EOF\-\-\-' )" ] && { rm -f $tmp_down_file ; logger -t "$scriptname[$$]" "错误！！！ 找不到 EOF ， base64 解码官方 gfwlist 数据不完整" ; }
		...
		logger -t "$scriptname[$$]" "错误！！！获取官方 gfwlist 下载失败"
		fi
		rm -f $tmp_down_file
	fi
	ss_3p_kool=`nvram get ss_3p_kool`
	if [ "$ss_3p_kool" = "1" ] ; then
		logger -t "$scriptname[$$]" "正在获取 koolshare 列表...."
	...
	if [ ! -z "$ss_sub5" ] ; then
		logger -t "$scriptname[$$]" "正在获取 GFW 自定义域名 列表...."
		...
		logger -t "$scriptname[$$]" "错误！！！获取 GFW 自定义域名 列表 下载失败"
		fi
		rm -f $tmp_down_file
	fi
	if [ ! -s $tmp_gfwlist ] ; then
		logger -t "$scriptname[$$]" "加入 固件内置list规则 列表...."
	...
	if [ "$ss_sub1" = "1" ] ; then
		logger -t "$scriptname[$$]" "处理订阅列表1....海外加速"
		url='/list.txt'
	...
	if [ "$ss_sub2" = "1" ] ; then
		#处理只做dns解释的域名
		logger -t "$scriptname[$$]" "处理订阅列表2....处理只做dns解释的域名"
		url='/dnsonly.txt'
		wgetcurl_checkmd5 $tmp_down_file "$hiboyfile$url" "$hiboyfile2$url" N 5
		if [ -s $tmp_down_file ] ; then
		echo ""  >> $tmp_down_file
	...
	if [ "$ss_sub3" = "1" ] ; then
		#处理需要排除的域名解释
		logger -t "$scriptname[$$]" "处理订阅列表3....处理需要排除的域名解释"
		url='/passby.txt'
		wgetcurl_checkmd5 $tmp_down_file "$hiboyfile$url" "$hiboyfile2$url" N 5
		if [ -s $tmp_down_file ] ; then
		echo ""  >> $tmp_down_file
	...
	if [ ! -z "$ss_sub6" ] ; then
		logger -t "$scriptname[$$]" "正在获取 GFW IP 列表...."
		...
		logger -t "$scriptname[$$]" "错误！！！获取 GFW IP 列表 下载失败"
		fi
		rm -f $tmp_down_file
	fi
	fi
	...
	logger -t "$scriptname[$$]" "完成下载 gfwlist 文件"
	rm -f $tmp_gfwlist
	rm -f /etc/storage/basedomain.txt
...
update_gfwlist_ipset() {
	...
	if [ ! -s $file_gfwlist_txt ] ; then
		logger -t "$scriptname[$$]" "错误！！！$file_gfwlist_txt 文件为空，使用 固件内置 /etc/storage/basedomain.txt 规则...."
		rm -f /etc/storage/basedomain.txt
		tar -xzvf /etc_ro/basedomain.tgz -C /tmp ; cd /opt
		ln -sf /tmp/basedomain.txt /etc/storage/basedomain.txt
	...
	logger -t "$scriptname[$$]" "开始加载 gfwlist 规则...."
	if is_md5_not ; then
	...
	if [ -s $file_gfwlist_txt ] ; then
		...
		logger -t "$scriptname[$$]" "配置更新，完成加载 gfwlist 规则...."
	else
		logger -t "$scriptname[$$]" "更新错误！！！ $file_gfwlist_txt 规则为空...."
	fi
	else
		[ ! -s $file_gfwlist_txt ] && logger -t "$scriptname[$$]" "匹配错误！！！ $file_gfwlist_txt 规则为空...."
		logger -t "$scriptname[$$]" "配置匹配，完成加载 gfwlist 规则...."
	fi
	fi
}
...
update_chnlist_file() {
	logger -t "$scriptname[$$]" "开始下载更新 chnlist 文件...."
	mkdir -p /opt/app/ss_tproxy/rule
	...
	logger -t "$scriptname[$$]" "完成下载 chnlist 文件"
	rm -f $tmp_down_file
}
...
update_chnlist_ipset() {
	...
	if is_chnlist_mode; then
		...
		logger -t "$scriptname[$$]" "开始加载 chnlist 规则（回国模式）...."
		...
		echo "" > /opt/app/ss_tproxy/dnsmasq.d/accelerated-domains.china.conf
		if [ -s /opt/app/ss_tproxy/rule/chnlist.txt ] ; then
		logger -t "$scriptname[$$]" "加速国内 dns 访问"
		logger -t "$scriptname[$$]" "开始加载 chnlist 规则...."
		wan_dnsenable_x="$(nvram get wan_dnsenable_x)"
		[ "$wan_dnsenable_x" == "1" ] && DNS_china=`nvram get wan0_dns |cut -d ' ' -f1`
		[ "$wan_dnsenable_x" != "1" ] && DNS_china=`nvram get wan_dns1_x |cut -d ' ' -f1`
		[ -z "$DNS_china" ] && DNS_china="$dns_direct"
...
update_chnroute_file() {
	...
	if [ "$1" != "ipv6" ]; then
	logger -t "$scriptname[$$]" "开始下载更新 chnroute 文件...."
	...
	if [ ! -z "$ss_sub7" ] ; then
		logger -t "$scriptname[$$]" "正在获取 ① 大陆白名单 IP 下载地址...."
		wgetcurl_checkmd5 $tmp_down_file $ss_sub7 $ss_sub7 Y
		if [ -s $tmp_down_file ] && [ ! -z "$(cat $tmp_down_file | grep -Eo [^A-Za-z0-9+/=]+ | tr -d "\n")" ] ; then
		echo ""  >> $tmp_down_file
		cat $tmp_down_file | grep -v '^#' | sort -u | grep -v "^$" >> $tmp_chnroute
		else
		logger -t "$scriptname[$$]" "错误！！！获取 ① 大陆白名单 IP 下载失败"
		fi
		rm -f $tmp_down_file
	fi
	...
	if [ ! -s $tmp_chnroute ] ; then
		...
		[ -s /tmp/china_ip_list.txt ] && logger -t "$scriptname[$$]" "错误！！！下载文件为空，使用 固件内置 /etc_ro/china_ip_list.tgz 规则...." && cat /tmp/china_ip_list.txt > $tmp_chnroute
	fi
	# 添加自定义白名单
	...
	logger -t "$scriptname[$$]" "完成下载 chnroute 文件"
	fi
	if is_true "$ipv6" || [ "$1" == "ipv6" ]; then
		rm -f $tmp_chnroute $tmp_down_file
		logger -t "$scriptname[$$]" "开始下载更新 chnroute6 文件...."
		...
		rm -f $tmp_down_file
		logger -t "$scriptname[$$]" "完成下载 chnroute6 文件"
	fi
}
...
update_chnroute_ipset() {
	...
	if [ ! -s $file_chnroute_txt ] ; then
		tar -xzvf /etc_ro/china_ip_list.tgz -C /tmp ; cd /opt
		[ -s /tmp/china_ip_list.txt ] && logger -t "$scriptname[$$]" "错误！！！ $file_chnroute_txt 文件为空，使用 固件内置 /etc_ro/china_ip_list.tgz 规则...." && cat /tmp/china_ip_list.txt > $file_chnroute_txt
	fi
	rm -f /etc/storage/china_ip_list.txt
	ln -sf $file_chnroute_txt /etc/storage/china_ip_list.txt
	...
	if [ "$1" != "ipv6" ]; then
	logger -t "$scriptname[$$]" "开始加载 chnroute 规则...."
	if is_true "$ipv4"; then
	...
	if is_md5_not ; then
		...
		echo "$chnroute_list" > /opt/app/ss_tproxy/tmp/chnroute_list_Number
		update_md5_check update_chnroute_txt $file_chnroute_txt /opt/app/ss_tproxy/tmp/chnroute_list_Number
		logger -t "$scriptname[$$]" "配置更新，完成加载 chnroute 规则...."
		else
		logger -t "$scriptname[$$]" "更新错误！！！ $file_chnroute_txt 规则为空...."
		fi
	else
		[ ! -s $file_chnroute_txt ] && logger -t "$scriptname[$$]" "匹配错误！！！ $file_chnroute_txt 规则为空...."
		logger -t "$scriptname[$$]" "配置匹配，完成加载 chnroute 规则...."
	fi
	else
	...
	if is_true "$ipv6" || [ "$1" == "ipv6" ]; then
	logger -t "$scriptname[$$]" "开始加载 chnroute6 规则...."
	...
	if is_md5_not ; then
		...
		echo "$chnroute6_list" > /opt/app/ss_tproxy/tmp/chnroute6_list_Number
		update_md5_check update_chnroute6_txt $file_chnroute6_txt /opt/app/ss_tproxy/tmp/chnroute6_list_Number
		logger -t "$scriptname[$$]" "配置更新，完成加载 chnroute6 规则...."
		else
		logger -t "$scriptname[$$]" "更新跳过！！！ $file_chnroute6_txt 规则为空...."
		fi
	else
		[ ! -s $file_chnroute6_txt ] && logger -t "$scriptname[$$]" "匹配错误！！！ $file_chnroute6_txt 规则为空...."
		logger -t "$scriptname[$$]" "配置匹配，完成加载 chnroute6 规则...."
	fi
	else
...
if [ "$ss_dnsproxy_x" = "0" ] ; then
	...
	logger -t "$scriptname[$$]" "启动 dnsproxy 防止域名污染"
	...
	logger -t "$scriptname[$$]" "错误 dnsproxy 没启动！"
...
logger -t "$scriptname[$$]" "启动 pdnsd 防止域名污染"
pidof dnsproxy >/dev/null 2>&1 && killall dnsproxy && killall -9 dnsproxy 2>/dev/null
pidof pdnsd >/dev/null 2>&1 && killall pdnsd && killall -9 pdnsd 2>/dev/null
...
logger -t "$scriptname[$$]" "错误 pdnsd 没启动！"
ss_dnsproxy_x=2 ; nvram set ss_dnsproxy_x=2
fi
...
if [ "$chinadns_port" = "8053" ] ; then
	...
	sleep 1
	logger -t "$scriptname[$$]" "使用 dnsmasq ，第三方 DNS 程序防止域名污染"
	if [ "$chinadns_enable" == "0" ] && [ "$chinadns_ng_enable" == "0" ] ; then
	logger -t "$scriptname[$$]" "自动开启 ChinaDNS-NG 防止域名污染"
	nvram set app_102=1
	fi
	...
	logger -t "$scriptname[$$]" "错误！！！ 第三方 DNS 程序 8053 端口没启动！"
...
start_iptables_redirect_mode() {
	...
	if is_ipv4_ipts $1; then
		...
		# 加载 kp过滤方案 规则
		logger -t "$scriptname[$$]" "设置内网(LAN)访问控制【kp过滤方案】"
...
for options in $arguments; do 
	[ "$options" != "h" ] && [ "$options" != "v" ] && logger -t "$scriptname[$$]" "$options"
	case "$options" in
		start)           start; status;;
		stop)            stop; status;;
